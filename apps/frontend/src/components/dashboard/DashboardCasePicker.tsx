...
/**
 * Graph-style case scope picker with an "All Cases" aggregate option.
 * `value` is `'all'` or a case_number (matches existing dashboard filter contract).
 */
export function DashboardCasePicker({
  ...
  const isAll = value === 'all' || !selectedCase;

  const label = isAll ? 'All Cases' : selectedCase.case_number;
  ...
  return (
    <div className={cn('relative min-w-0', className)} ref={rootRef}>
      <button
        ...
        className={cn(
          'flex w-full items-center gap-2 rounded-xl border text-left transition',
          compact ? 'max-w-[14rem] px-2 py-1' : 'max-w-md px-2.5 py-1.5',
          open
      ...
      {open && (
        <div
          className={cn(
            surfaceCard,
            'absolute right-0 top-full z-50 mt-2 w-[min(22rem,calc(100vw-3rem))] overflow-hidden p-0'
          )}
        >
              ...
              filtered.map((c) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => selectValue(c.case_number)}
                    className={cn(
                      'flex w-full items-start gap-2 rounded-lg px-2.5 py-2 text-left transition',
